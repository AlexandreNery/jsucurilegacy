...
//		int samples = new Integer(args[5]).intValue();
		...
		NodeFunction rayCast = new NodeFunction() {
			...
			public Object f(Object[] in_operands) {
				int threadId = (int) in_operands[0];
				float[] data = (float[]) in_operands[1];
				Camera camera = (Camera) in_operands[2];
				int samplesRayCast = (int) in_operands[3];
				Grid gridRayCast = (Grid) in_operands[4];
				int numRayCastNodesRayCast = (int) in_operands[5];
...
//		};
		...
		NodeFunction readImage = new NodeFunction() {
			...
			public Object f(Object[] in_operands) {
				float[] data = null;
				String filePath = (String)in_operands[0];
				int[] dimensions = (int[])in_operands[1];
...
//		};
		...
		NodeFunction writeImage = new NodeFunction() {
			...
			public Object f(Object[] in_operands) {
				...
				Camera camera = (Camera)in_operands[in_operands.length - 2];
				int numberRayCastNodes = (int)in_operands[in_operands.length - 1];
				...
				for(int indexNodes = 0; indexNodes < numberRayCastNodes; indexNodes++){
					...
					int end = start + chunck < camera.getWidth() ? start + chunck : camera.getWidth();
					List<java.awt.Color> colorListPartial = (ArrayList<java.awt.Color>)in_operands[indexNodes];
